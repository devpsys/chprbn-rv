...
| 2 | Strings to `strings.xml` + accessibility pass | Medium | 🟡 Partial — `strings.xml` foundation sectioned + populated. Auth flow (Splash, Login, LoginViewModel error) and shared design-system (AppTopBar, BottomNavBar) migrated. Pattern + VM Context-injection recipe established (§9 of `CODE_REVIEW.md` follow-up). Remaining detail screens (Verification, Profile, Dashboard, Sync, Exam, Scan) still hold ~150+ hardcoded literals — mechanical follow-up sprint. Accessibility (U1) is a separate item. |
