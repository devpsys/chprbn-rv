...
| 2 | Strings to `strings.xml` + accessibility pass | Medium | 🟡 All screens migrated; small follow-ups remain — `strings.xml` foundation sectioned + populated (~220 keys). Migrated: auth (Splash + Login + LoginViewModel), design-system (AppTopBar + BottomNavBar + ExamBottomNavBar), top-level nav (UnifiedDashboard + Verification + Profile), VerifiedListScreen, ManualEntryScreen, RecordDetailScreen, VerificationFormScreen + VM, ReportIrregularityScreen + VM, SyncScreen, SyncHistoryScreen, QrScanScreen + AppNavHost scan label, all six Exam* screens (ExamDashboard / ExamPapers / ExamPaper / ExamCandidates / ExamStatistics / CandidateScanResult). **Remaining (cross-layer follow-ups, not @Composable scope)**: Officer-remark dropdown OPTIONS list (needs a `<string-array>` + Context-backed loader in `VerificationFormViewModel`), `IrregularityRemark.displayLabel` domain-enum field, `VerificationRepositoryImpl.getFeatures()` static titles, and `CandidateScanResultUiState.fromScannedPayload` mock-data labels. Accessibility (U1) is a separate item. |
