...
| 2 | Strings to `strings.xml` + accessibility pass | Medium | 🟡 Most non-exam screens migrated — `strings.xml` foundation sectioned + populated (~165 keys). Migrated: auth (Splash + Login + LoginViewModel), design-system (AppTopBar + BottomNavBar), top-level nav (UnifiedDashboard + Verification + Profile), VerifiedListScreen, ManualEntryScreen, RecordDetailScreen, VerificationFormScreen + VM, ReportIrregularityScreen + VM, SyncScreen, SyncHistoryScreen, QrScanScreen + AppNavHost scan label. **Remaining**: all six Exam* screens (ExamDashboard / ExamPapers / ExamPaper / ExamCandidates / ExamStatistics / CandidateScanResult), the Officer-remark dropdown OPTIONS list (needs a string-array + Context-backed loader in `VerificationFormViewModel`), the IrregularityRemark.displayLabel domain-enum field (cross-layer concern), and `VerificationRepositoryImpl.getFeatures()` static titles. Accessibility (U1) is a separate item. |
