...
| 2 | Strings to `strings.xml` + accessibility pass | Medium | 🟡 Substantial progress — `strings.xml` foundation sectioned + populated (~115 keys). Migrated: auth (Splash + Login + LoginViewModel), design-system (AppTopBar + BottomNavBar), top-level nav (UnifiedDashboard + Verification + Profile), VerifiedListScreen, ManualEntryScreen, RecordDetailScreen, VerificationFormScreen + VM error. Remaining: ReportIrregularityScreen + VM, SyncScreen, SyncHistoryScreen, QrScanScreen, all six Exam* screens, and the Officer-remark dropdown OPTIONS list (needs a string-array + Context-backed loader). Accessibility (U1) is a separate item. |
