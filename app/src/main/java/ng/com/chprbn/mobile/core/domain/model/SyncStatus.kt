 ...
 * attempt failed and the row should be retried; `Abandoned` marks a row that
 * has exceeded [ng.com.chprbn.mobile.core.sync.SyncBatchRunner.MAX_ATTEMPTS]
 * failed attempts — it is left in place for auditability but the runner will
 * not pick it up again (E4 dead-letter).
 *
 ...
 */
enum class SyncStatus { Pending, Synced, Failed, Abandoned }
